...
# ### 问题 7 - 回答:  
# 网格搜索算法是一种通过遍历给定的参数组合来优化模型表现的方法。参数通过交叉验证的方法进行优化模型。
...
# ### 问题 10 - 回答：  
# 建议售价如上。从房屋特征来看，房间总数越高，房子成本越高；社区贫困指数越低表示区域内富人比较聚集，社区贫困指数越高表示穷人较为聚集，贫富水平也会间接影响房价；同时，学生/老师比例可以间接反映一个地区的教育水平高低，教育水平高的地区更易吸引有孩子的家庭在附近购房，所以学生-老师比例也会在一定程度下影响房价。综上所诉，该模型是合理的。
